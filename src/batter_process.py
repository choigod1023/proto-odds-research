"""타선의 과정 지표 — 과거 투수 가설을 타자에 적용한 탐색 기록.

왜
--
과거에는 ERA(결과) → FIP(과정) → xFIP(과정+잡음축소)로 갈수록 Brier가
+0.00257 → +0.00432 → +0.00597로 개선된 것처럼 보였다. xFIP의 미래 리그평균과
같은 날짜 순서 누수를 제거한 최신 재실험은 시장 대비 적중 −0.24%p·Brier 악화였고,
이 과거 계보는 더 이상 예측 근거가 아니다.
...
DETAIL = ROOT / "data" / "raw" / "detail" / "kbo_batters_2023_2026.json"
...
def main() -> int:
    if not DETAIL.exists():
        print(f"{DETAIL} 없음 — python src/game_detail.py batters kbo 2023 2026")
        return 1
    d = load()
