...
`kbo_batters_2023_2026.json` — **팀 합계**다. 즉 이렇게 잰 셈이다.
    ...
    투수: 개인 단위(선발 xFIP)  →  과거 누수 실험에서 +0.006, 박빙 +0.014
    타선: 팀 시즌 평균          →  과거 실험에서 ≈0

위 xFIP 수치는 미래 리그평균·같은 날짜 순서 누수 제거 후 무효가 됐다. 최신 인과
재실험은 시장 대비 적중 −0.24%p와 Brier 악화로 끝났고 승격하지 않았다. 따라서 이를
"투수는 통했다"는 근거로 쓰지 않는다. 타선을 개인 단위로 재는 가설만 별도로 검사한다.
