...
def parse_baseball_batters(res: dict) -> dict | None:
    ...
    투수 FIP·xFIP에 맞춰 타선도 개인 과정 지표로 만드는 수집기다. 다만 xFIP의
    과거 개선치는 시간누수 제거 후 무효가 됐으므로 투수 모델의 우위를 전제하지 않는다.
...
def parse_baseball_batters_indiv(res: dict) -> dict | None:
    ...
    **그날 실제로 나온 9명이 누구인지는 한 번도 보지 않았다.**
    ...
    투수 개인 단위 xFIP가 통했다는 +0.006은 시간누수로 무효다. 그래도 타선을 팀
    단위로만 재던 측정 비대칭은 별개이므로, 이 파서는 개인 단위 자료를 남긴다.
