...
from src.recommendation_policy import MAX_AUTO_RECOMMENDATION_ODDS
...
def test_browser_and_generator_recommendation_thresholds_stay_in_sync():
    ...
    hit_match = re.search(
        r"export const DAILY_CHALLENGE_MIN_HIT = \{\s*1\.4:\s*([\d.]+),\s*2:\s*([\d.]+)\s*\};",
        plan_js,
    )
    assert hit_match, "브라우저 목표별 적중 하한을 찾을 수 없다"
    assert {1.4: float(hit_match.group(1)), 2: float(hit_match.group(2))} == DAILY_CHALLENGE_MIN_HIT
