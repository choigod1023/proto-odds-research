...
import src.internal_probability as internal_module
...
def _game(lineup_state="official_today"):
    return {
        ...
        "선발": {
            ...
            "first_seen_at": "2026-08-27T08:00:00+09:00", "revision_id": "revision-1",
...
def test_confirmed_starter_and_lineup_move_baseball_probability(monkeypatch):
    monkeypatch.setattr(internal_module, "league_is_promoted", lambda league: True)
...
def test_missing_first_seen_revision_fails_closed():
    game = _game()
    game["선발"].pop("first_seen_at")
    result = internal_probability(game, {
        "market": "승패", "선택": "홈", "시장확률": .50, "모델확률": .60})
    assert result["status"] == "ineligible"
    assert result["reason"] == "player_first_seen_revision_missing"


def test_unvalidated_league_keeps_internal_candidate_shadow_only():
    result = internal_probability(_game(), {
        "market": "승패", "선택": "홈", "시장확률": .52, "모델확률": .55})
    assert result["status"] == "shadow_only"
    assert result["final"] == .52
    assert result["shadow_final"] != result["final"]


@pytest.mark.parametrize("league", ["MLB", "KBO", "NPB"])
def test_supported_baseball_leagues_share_activation_contract(league, monkeypatch):
    monkeypatch.setattr(internal_module, "league_is_promoted", lambda value: True)
    game = _game()
    game["league"] = league
    result = internal_probability(game, {
