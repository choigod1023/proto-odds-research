...
const game = {
  ...
  선발: {
    ...
    home_detail: {
      name: "마츠모토 하루",
      stats: { period: "2026시즌", record: "8승 4패", era: 2.5, whip: .99, strikeouts: 90 },
    },
...
test("NPB 요약에 양 팀 선발투수 성적과 대표 타자를 함께 넣는다", () => {
  const summary = playerSummaryFor(game);
  assert.deepEqual(summary.players.map((player) => player.name),
    ["마츠모토 하루", "후루샤 다쿠미", "야마카와 호타카", "무라바야시 이쓰키"]);
...
test("오늘 공식 타순이면 요약에서도 공식 상태를 명시한다", () => {
  const official = structuredClone(game);
  official.선발.lineup_status.state = "official_today";
